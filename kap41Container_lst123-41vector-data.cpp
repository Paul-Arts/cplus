    ...
    int main() {
      ...
      { // Schreiben
        auto out = fopen(FILENAME, "wb"); // Datei mit C zum Schreiben ?ffnen
        if(out==nullptr) {
        ...
        fclose(out); // In C muss man explizit schlie?en. Ehrlich.
      }

      vector<int> gelesen{};
      { // Lesen
        auto in = fopen(FILENAME, "rb"); // Datei mit C zum Lesen ?ffnen
        if(in==nullptr) {
            cerr << "Fehler beim Oeffnen\n"; return -1;
        }
        const size_t sz = 4; // angenommen, wir wissen, wir lesen 4 Elemente ...
        gelesen.resize(sz); // Platz schaffen f?r zu lesende Daten
