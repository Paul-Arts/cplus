    ...
    namespace {                          // Beginn des anonymer Namensraums
    ...
    bool Ship::fireUpon(shared_ptr<Asteroid> a) {
        if(!a) return false;             // ung?ltiger Asteroid
        a->structure_ -= firepower;
        if(a.get() == firedLastOn_.get())
            firepower *= 2 ;             // Schaden vergr??ern
        else
            firepower = 1;               // zur?cksetzen
        firedLastOn_ = a;
    ...
    int wuerfel(int min, int max) {
        ...
        return std::uniform_int_distribution<int>{min, max}(e); // w?rfeln
    }
    ...
    int main() {
        ...
        Ship ship{};
        for(int idx = 0; idx < 85; ++idx) {                     // 85 Sch?sse
