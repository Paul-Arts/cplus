    ...
    public:
        ...
        unique_ptr<Button> showModal()
            { return std::move(btnOk_); }    // Platzhalter-Code; OK gedr?ckt
    };
    unique_ptr<MyDialog> createDialog() {
        return unique_ptr<MyDialog>{ // tempor?rer Wert
            new MyDialog{"Bitte Namen eingeben"}};
    }
    int showDialog() {
        unique_ptr<MyDialog> dialog = createDialog();       // lokale Variable
        unique_ptr<Button> gedrueckt = dialog->showModal(); // R?ckgabewert
    ...
    int main() {
        ...
        if(gedrueckt_id == 1) {
            ...
            //= Danke, dass Sie OK gedrueckt haben
        }
    }
